...
class GraphAPI(object):
    ...
    def request(self, path, args=None, post_args=None):
        ...
        try:
            ...
            # print(e)
            # response = _parse_json(e.read().decode())
            # print(response)
            # raise GraphAPIError(response)
            file = e
        except TypeError:
        ...
        try:
            ...
            fileInfo = file.info()
            # print(fileInfo)
            fileInfo_type = fileInfo.get_content_maintype()
            # print(fileInfo_type)
            # if fileInfo.maintype == 'text':
            if fileInfo_type == 'text':
                response = _parse_json(file.read().decode())
            # elif fileInfo.maintype == 'image':
            elif fileInfo_type == 'image':
...
def get_app_access_token(app_id, app_secret):
    ...
    try:
        # result = file.read()
        result = file.read().decode().split("=")[1]
